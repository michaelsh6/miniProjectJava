...
public class CameraIntegrationTest {
    ...
    public int CountCameraAndGeometriesIntersectionTest(Geometry geometryShape){
        ...
        for (int i=0; i<nX; i++)
        {
            for (int j=0; j<nY; j++)
            {
                ...
                count += geometryShape.findIntersections(ray).size();
            }
        }
        return count;
    }
    ...
    public void TestConstructRayThroughPixelPlane(){
        ...
        assertEquals("num of intersection is not correct", 9, CountCameraAndGeometriesIntersectionTest(new Plane
        ...
        assertEquals("num of intersection is not correct", 9, CountCameraAndGeometriesIntersectionTest(new Plane
                (new Point3D(1,1,2),new Point3D(-1,1,2) ,new Point3D(0,0,3))));

        //T03 plane is horizontal to 'toward' camera vector, and only few rays cross the plane (few are parallels)
        assertEquals("num of intersection is not correct", 6, CountCameraAndGeometriesIntersectionTest(new Plane
